...
def allocate_mem(
    input_batch, batch_size=32, output_classes=12, target_dtype=np.float16
):
    output = np.empty(
        [batch_size, output_classes], dtype=target_dtype
    )  # Need to set output dtype to FP16 to enable FP16
...
def predict(
    X_batch, d_input, d_output, bindings, stream, context, output, batch_size
):
    st = time.time()
    # Transfer input data to device
